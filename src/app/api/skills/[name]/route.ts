...
function buildInstalledSkillPlaceholderContent({
  ...
  return [
    `# ${name}`,
    "",
    "This installed skill does not include a root `SKILL.md`.",
    "",
    `- Runtime: ${runtimeLabel}`,
    `- Directory: ${directoryPath}`,
    "",
    "Deleting this entry will uninstall the skill from the runtime.",
  ].join("\n");
}
...
function parseSkillFrontMatter(content: string): { name?: string; description?: string } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (nameMatch) {
      result.name = normalizeFrontMatterScalar(nameMatch[1]);
      continue;
    }
    ...
    if (descMatch) {
      result.description = normalizeFrontMatterScalar(descMatch[1]);
    }
  }
  return result;
}
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        matches.push({
          ...
          directoryPath: skillRoot,
          installedSource: source,
          contentHash: `virtual:${source}:${entry.name}`,
          skillId: entry.name,
          virtual: true,
        });
      }
...
export async function GET(
  ...
  try {
    ...
    const content = found.virtual
      ? buildInstalledSkillPlaceholderContent({
          name,
          installedSource: found.installedSource || "claude",
          directoryPath: found.directoryPath || path.dirname(found.filePath),
        })
      : fs.readFileSync(found.filePath, "utf-8");
...
export async function PUT(
  ...
  try {
    ...
    const targetPath =
      found.source === "installed" && found.virtual
        ? path.join(found.directoryPath || path.dirname(found.filePath), "SKILL.md")
        : found.filePath;
...
export async function DELETE(
  ...
  try {
    ...
    if (found.source === "installed" && found.installedSource) {
      const runtime = found.installedSource === "agents" ? "codex" : "claude-code";
      const uninstallTarget = found.skillId || name;
      const args = ["remove", uninstallTarget, "-g", "-y", "--agent", runtime];
      const result = spawnSkillsCliSync(args, {
        env: { ...process.env },
        encoding: "utf-8",
      });
      ...
      if (result.status !== 0) {
        return NextResponse.json(
          { error: stderrText.trim() || stdoutText.trim() || `Failed to uninstall skill ${uninstallTarget}` },
          { status: 500 }
        );
      }
