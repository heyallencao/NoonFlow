...
function buildInstalledSkillPlaceholderContent({
  ...
  const lines = [
    `# ${name}`,
    "",
    "This installed skill does not include a root `SKILL.md`, so NoonFlow generated this summary from the local installation metadata.",
    "",
  ...
  if (installedAt) {
    lines.push(`- Installed at: ${installedAt}`);
  }

  lines.push(
    "",
    "Edit this entry to create a local `SKILL.md` alongside the installed files."
  );
...
function scanInstalledSkillPlaceholders(
  dir: string,
  installedSource: InstalledSource
): InstalledSkill[] {
  const placeholders: InstalledSkill[] = [];
  if (!fs.existsSync(dir)) return placeholders;
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const lockEntry = lockFile.skills[entry.name];
      if (!lockEntry) continue;

      placeholders.push({
        name: entry.name,
        description: `Installed from ${lockEntry.source}`,
        content: buildInstalledSkillPlaceholderContent({
          name: entry.name,
...
function resolveInstalledSkills(
  ...
  const resolved: ResolvedInstalledSkill[] = [];
  for (const group of byName.values()) {
    ...
    resolved.push(
      ...group.map((skill) => ({
        ...skill,
        runtimeAvailability: getRuntimeAvailability(skill.installedSource),
      }))
    );
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const claudeSkills = [
      ...scanInstalledSkills(claudeSkillsDir, "claude"),
      ...scanInstalledSkillPlaceholders(claudeSkillsDir, "claude"),
    ];
