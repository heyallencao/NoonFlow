...
import { publishAttachFileToChat, subscribeOpenFilePreview } from '@/lib/events/app-event-bus';
...
export function RightPanel({ width }: RightPanelProps) {
  ...
  return (
    <aside
      className="relative hidden h-full shrink-0 flex-col overflow-hidden bg-bg-primary text-sidebar-foreground lg:flex shadow-[-4px_0_24px_-12px_rgba(0,0,0,0.2)] z-10"
      style={{ width: width ?? 288 }}
    >
      <div className="relative flex h-full flex-col">
        {/* Header - Unified spacing and typography */}
        <div className="flex h-14 shrink-0 items-center justify-between px-4">
          <span className="text-xs font-semibold uppercase tracking-[0.08em] text-sidebar-foreground/60">
            {t('panel.tasksAuto')}
          </span>
          <div className="flex items-center gap-2">
            ...
            <Tooltip>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    activeInspector === 'tasks'
                      ? 'border-emerald-400/50 bg-emerald-500/12 text-emerald-200 shadow-sm'
                      : 'border-transparent bg-foreground/5 text-sidebar-foreground/75 hover:bg-foreground/10 hover:text-sidebar-foreground',
                  )}
              ...
              <TooltipContent side="left" className="text-xs">{t('panel.tasks')}</TooltipContent>
            </Tooltip>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  className={cn(
                    'relative inline-flex h-8 w-8 items-center justify-center rounded-lg border transition-all duration-200',
                    activeInspector === 'permissions'
                      ? 'border-amber-400/50 bg-amber-500/12 text-amber-200 shadow-sm'
                      : 'border-transparent bg-foreground/5 text-sidebar-foreground/75 hover:bg-foreground/10 hover:text-sidebar-foreground',
                  )}
                ...
                <button
                  ...
                  className={cn(
                    ...
                    activeInspector === 'tools'
                      ? 'border-blue-400/50 bg-blue-500/12 text-blue-200 shadow-sm'
                      : 'border-transparent bg-foreground/5 text-sidebar-foreground/75 hover:bg-foreground/10 hover:text-sidebar-foreground',
                  )}
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-transparent bg-foreground/5 text-sidebar-foreground/75 transition-all duration-200 hover:bg-foreground/10 hover:text-sidebar-foreground"
            ...
            <div className="pointer-events-auto max-h-[min(60vh,480px)] w-full overflow-auto rounded-xl border border-border-default bg-background p-4 shadow-xl">
              {activeInspector === 'tasks' ? (
                <section className="space-y-4">
                  <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.08em] text-sidebar-foreground/60">
