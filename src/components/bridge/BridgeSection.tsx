...
const BRIDGE_FEATURE_DISABLED = true;
...
export function BridgeSection() {
  ...
  const handleBrowseFolder = async () => {
    try {
      ...
      // Desktop bridge unavailable or folder dialog unsupported
    }
  };
  ...
  const isConfiguredEnabled = settings.remote_bridge_enabled === "true";
  const isEnabled = !BRIDGE_FEATURE_DISABLED && isConfiguredEnabled;
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <Switch
            ...
            disabled={saving || BRIDGE_FEATURE_DISABLED}
          />
        </div>
        {BRIDGE_FEATURE_DISABLED ? (
          <div className="mt-3 flex items-start gap-2 rounded-md border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-[13px] text-amber-700 dark:text-amber-300">
            <HugeiconsIcon icon={Alert02Icon} className="mt-0.5 h-3.5 w-3.5 shrink-0" />
            <div className="space-y-0.5">
              <p className="font-medium">{t("bridge.previewNoticeTitle")}</p>
              <p className="text-amber-700/80 dark:text-amber-200/80">
                {t("bridge.previewNoticeDesc")}
              </p>
            </div>
          </div>
        ) : isEnabled ? (
          <div className="mt-3 flex items-center gap-2 rounded-md bg-blue-500/10 px-3 py-2 text-[13px] text-blue-600 dark:text-blue-400">
      ...
      {isEnabled && (
        <div className="rounded-2xl border border-border/40 bg-card p-6 shadow-sm transition-all hover:shadow-md space-y-4">
