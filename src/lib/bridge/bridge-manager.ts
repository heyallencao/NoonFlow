 ...
 * Called from POST /api/bridge with action 'auto-start' (triggered by desktop runtime startup).
 */
    ...
    while (state.running && adapter.isRunning()) {
      try {
        ...
        if (msg.callbackData || msg.text.trim().startsWith('/')) {
          ...
          processWithSessionLock(binding.sessionId, () =>
            handleMessage(adapter, msg),
          ).catch(err => {
            console.error(`[bridge-manager] Session ${binding.sessionId.slice(0, 8)} error:`, err);
          });
        }
      } catch (err) {
...
async function handleMessage(
  ...
  const state = getState();
  state.activeTasks.set(binding.sessionId, taskAbort);
  ...
  try {
    ...
    state.activeTasks.delete(binding.sessionId);
...
async function handleCommand(
  ...
  switch (command) {
    ...
    case '/sessions': {
      ...
      if (bindings.length === 0) {
        ...
        for (const b of bindings.slice(0, 10)) {
          ...
          lines.push(`<code>${b.sessionId.slice(0, 8)}...</code> [${active}] ${escapeHtml(b.workingDirectory || '~')}`);
        }
        response = lines.join('\n');
      }
      break;
    ...
    case '/stop': {
      const binding = router.resolve(msg.address);
      const st = getState();
      const taskAbort = st.activeTasks.get(binding.sessionId);
      if (taskAbort) {
        taskAbort.abort();
        st.activeTasks.delete(binding.sessionId);
