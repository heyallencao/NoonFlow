/**
 * Telegram Bot notification module for NoonFlow.
 *
...
export async function notifyPermissionRequest(
  ...
  const msg = [
    ...
    ``,
    `⚠️ Please approve or deny in NoonFlow.`,
  ].filter(Boolean).join('\n');
...
/**
 * Build a status summary of current NoonFlow sessions.
 ...
 * Supports: /status — replies with current NoonFlow session status.
        ...
        if (command === '/start') {
          await callTelegramApi(botToken, 'sendMessage', {
            ...
            text: [
              `👋 <b>NoonFlow Bot</b>`,
              ``,
              `I'll send you notifications about your NoonFlow tasks.`,
              ``,
              `<b>Commands:</b>`,
